...
The CursorFlow backend is a **MCP-first Node.js/TypeScript application** providing serverless API services with **MCP (Model Context Protocol) integration as the primary approach**. It integrates with multiple LLM providers, v0 Platform, and Supabase (Postgres) for data persistence.
...
## 🏗️ Service Architecture (MCP-First)
...
> **CLI Option Documentation**: For CLI-specific backend implementation details, see [`../cli-option/CLI_ARCHITECTURE.md`](../cli-option/CLI_ARCHITECTURE.md).

---

*This implementation provides a robust foundation for rapid UI generation through MCP commands with comprehensive monitoring and error handling, all within the Cursor IDE environment.* 