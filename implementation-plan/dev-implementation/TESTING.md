...
- **V0 API Testing**: V0 SDK + Mock responses
- **MCP Server Testing**: MCP protocol testing
...
- **V0 Mocking**: V0 API response mocks
- **MCP Testing**: MCP protocol test utilities

## 🧪 Unit Testing

### **V0 Integration Tests**
```typescript
// tests/services/v0-integration.test.ts
import { V0IntegrationService } from '../services/v0-integration';
import { mockV0Response } from '../mocks/v0-api';

describe('V0IntegrationService', () => {
  let v0Service: V0IntegrationService;

  beforeEach(() => {
    v0Service = new V0IntegrationService();
  });

  it('should generate UI components successfully', async () => {
    const prompt = 'Create a todo app with dark mode';
    const targetDir = '/tmp/test-project';
    
    const result = await v0Service.generateUI(prompt, targetDir);
    
    expect(result.files).toBeDefined();
    expect(result.previewUrl).toBeDefined();
    expect(result.performance.generationTime).toBeLessThan(30000);
  });
  ...
  it('should handle V0 API errors gracefully', async () => {
    ...
    await expect(v0Service.generateUI(prompt, targetDir))
      .rejects.toThrow('V0 API error');
  });
});
```
...
### **MCP Server Tests**
```typescript
// tests/mcp/v0-mcp-server.test.ts
import { V0MCPServer } from '../mcp/v0-mcp-server';

describe('V0MCPServer', () => {
  let mcpServer: V0MCPServer;

  beforeEach(() => {
    mcpServer = new V0MCPServer();
  });

  it('should handle V0 generation requests', async () => {
    const request = {
    ...
    const response = await mcpServer.handleV0Request(request);
...
describe('MCPServer', () => {
  it('should handle MCP commands successfully', async () => {
    const mcpServer = new MCPServer();
    const command = '/nw build "Create a todo app"';
    
    const result = await mcpServer.executeCommand(command);
...
## **🚫 CLI & Extension Testing (Commented Out - Only When Required)**
...
```typescript
// tests/platforms/npx-cli.test.ts
import { NPXCLI } from '../platforms/npx-cli';

describe('NPXCLI', () => {
  it('should scaffold project successfully', async () => {
    const cli = new NPXCLI();
    const projectName = 'test-project';
    
    const result = await cli.scaffoldProject(projectName);
...
```typescript
// tests/platforms/vscode-extension.test.ts
import { VSCodeExtension } from '../platforms/vscode-extension';

describe('VSCodeExtension', () => {
  it('should generate UI via command palette', async () => {
    const extension = new VSCodeExtension();
    const prompt = 'Create a dashboard';
    
    const result = await extension.generateUI(prompt);
...
describe('CLI Integration', () => {
  it('should scaffold project via CLI', async () => {
    const { stdout } = await execAsync('npx nuggetwise-v0-init test-project');
...
describe('VS Code Extension Integration', () => {
  it('should integrate with VS Code command palette', async () => {
    const extension = new VSCodeExtension();
    ...
    const mockVSCode = {
      ...
      window: {
        showInputBox: jest.fn().mockResolvedValue('Create a todo app'),
        showInformationMessage: jest.fn(),
      },
    };
    
    extension.activate(mockVSCode);
    
    expect(mockVSCode.commands.registerCommand).toHaveBeenCalledWith(
      'nuggetwise.generateUI',
      expect.any(Function)
    );
  });
});
